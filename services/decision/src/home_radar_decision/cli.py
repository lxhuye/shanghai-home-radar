...
def enqueue_main() -> None:
    parser = argparse.ArgumentParser(description="Enqueue P5.5 decision orchestration")
    ...
    queue = Queue(settings.decision_queue, connection=Redis.from_url(settings.redis_url))
    job = queue.enqueue(
