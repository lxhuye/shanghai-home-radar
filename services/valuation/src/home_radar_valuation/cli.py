...
def enqueue_main() -> None:
    parser = argparse.ArgumentParser(description="Enqueue cached P4 listing valuations")
    ...
    queue = Queue(settings.valuation_queue, connection=Redis.from_url(settings.redis_url))
    job = queue.enqueue(
