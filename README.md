...
## 下一步：云端小范围采集验收（计划，尚未完成）

我们希望每天回答一个具体问题：**今天新增或降价的房源里，哪些值得继续跟踪，为什么？** 先证明一条真实数据链路，再扩大范围和部署调度。
...
这条云端链路是待实现与验收的目标架构，不是现有功能。拟先用 ChatGPT Work Cloud 做小范围人工监督试验；Codex with ChatGPT 的工作区连接不能当作数据库写入接口，也不等于将本地服务部署到云端。云端任务无法直接访问本机文件或 `127.0.0.1`。首轮可以人工下载私有产物并导入，但必须标记为人工中转，不能算作无人值守闭环。

### 第一轮验收

1. **T0 真实读取**：一个已明确范围的公开页面，采集约 10–20 套可复核房源；保留原始字段、来源 ID、URL、时区明确的观测时间和采集状态。未知字段保留空值，不用搜索摘要或旧文件冒充新观测。
2. **结构化与保存**：输出 JSON/CSV 和 SHA-256 清单，存入私有位置。校验字段、数量、唯一性与来源；维持 `sample / partial / private_research_only`，不改动冻结校准集。
3. **T1 再次观察**：按来源允许的低频节奏重新访问同一范围。区分首次见到、报价变化、无变化与读取失败。没有真实变化就如实报告，不制造降价；单页缺失不推断下架。
4. **进入池子**：复用现有 Collector 的校验与历史事件机制；重复导入不增生记录，新观测可追溯到来源和批次。没有入库记录，仅有网页答案不算通过。
5. **交付候选**：显示原始链接、观测时间、变化和跟踪理由。缺少估值或未来证据时进入待补证清单，不伪造分数；过期输入不能包装为今日机会。

只有以上验收通过，再接通有身份认证的私有云端接收端、持久化存储和定时任务，并做连续七天的运行验证。本地电脑关机后的入库与通知必须单独验证；仅把浏览器放到云端不能解决本地数据库离线。

### 尚待解决的壁垒

- **网站访问**：云端可读性、登录状态和人工接管需实际验证。遇验证码或访问限制暂停；不绕过验证、不承诺全站覆盖。
- **跨运行状态**：保留独立的观测历史和幂等键，不能依赖临时云端工作目录或聊天文字充当数据库。
- **安全接收**：现有本地接管接口不是公网收件服务；认证、重放保护、格式与体积限制、审计和失败重试仍需补齐。
- **可用性与成本**：云端工具的账户可用性、额度、运行耗时与失败率需要实测，暂不承诺免费或全天候稳定。
- **分析有效性**：新数据进入池子不等于推荐可信。继续遵守人工盲测和模型冻结，不为榜单非空调整权重或门槛。

欢迎先贡献来源适配、私有数据中转、幂等入库和端到端验收。第一里程碑是可复核的小闭环，不是新增预测模型。
